...
export const LAB_EXTRACTION_SYSTEM_PROMPT: string = [
  ...
  "- Reference range: when the document prints a TWO-SIDED numeric reference range for a measurement (e.g. 30-100), copy its two bounds into referenceRangeLow and referenceRangeHigh exactly as printed, in the SAME unit as the value. For a one-sided range (e.g. \"<200\" or \">40\"), a qualitative range, or a range printed in a DIFFERENT unit than the value, set BOTH referenceRangeLow and referenceRangeHigh to null (still keep referenceRangeText).",
  "- Optimal range: set optimalRangeLow and optimalRangeHigh to a commonly cited OPTIMAL WELLNESS band for generally healthy adults, drawn from your own general knowledge (NOT from the document), in the SAME unit as the value. This is wellness framing for a healthy lifestyle — NEVER a diagnostic threshold, treatment target, or medical-certainty cutoff, and never implies disease, deficiency, or a clinical decision. Set BOTH to null when you are not confident of a unit-matched wellness band.",
  "- For every range pair, set both bounds or set both to null, and keep low strictly less than high.",
