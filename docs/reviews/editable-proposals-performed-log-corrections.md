...
> **Status 2026-06-04:** C1–C7, C9, C10 + all verified dead-code/duplicate items **DONE** across correction workflows WF-A (C1/C6/C9), WF-B (C2/C3/C7), WF-C (C4/C5/C10 + dead exports + `_intent` param + ProposalCardShell + recipes-hook). Full CI green (types 791, api 1053, web 719).
> **Still open:** **C8** (stub `log_workout_activity` literal dedup → `buildStubLogWorkoutActivityProposal`) — not yet scheduled; `nutrition.repository.ts:274` `incidentDate` UTC-slice left as `TODO(C2)` (timezone unavailable at repo layer).
...
- ☐ **C8** `packages/ai/src/stub-provider.ts:232/243` — log_workout_activity proposal literal duplicated across `generateCoachResponse` & `generateDomainStep`. Extract `buildStubLogWorkoutActivityProposal(normalized)` into `stub-workout-plan.ts`; callers keep their own envelope. Risk: low.
