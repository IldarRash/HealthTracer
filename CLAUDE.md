...
- Attachments are **context-only**: there is no recognition/classification machinery (the multimodal domain LLMs read attachment content directly). Medical attachments are consent-gated at upload by the **user-declared category/document-type + MIME**; a save is a consent-gated proposal and must never create or parse `health_documents` rows. The removed recognizers/classifiers, `prepare_proposal_candidates` stage, and attachment proposal side-channel must not be reintroduced (see "Removed Legacy Paths" in the pipeline doc).
