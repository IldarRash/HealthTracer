...
NestJS modular monolith, one module per domain (`ai`, `chat`, `chat-attachments`, `coaching-context`, `proposals`, `workouts`, `nutrition`, `goals`, `habits`, `today`, `recovery`, `profiles`, `biomarkers`, etc.). Within a module: **controllers stay thin → services own domain logic → repositories own DB access.** Validate all API inputs and AI outputs with Zod contracts from `packages/types`. Use typed errors / Nest exceptions; never swallow failures.
...
ChatService.sendMessage           apps/api/src/modules/chat/chat.service.ts
  ...
  → ProposalNormalization          proposals/proposal-normalization.service.ts — per-intent trusted server-side stamping (imageRefs from turn attachments, provenance, date clamps; workout name→catalog bridge)
  → ProposalValidation (+ repair)  proposals/proposal-validation.service.ts — full stack; schema-class failures get one payload-only self-repair LLM call (ai/proposal-repair.service.ts), then re-normalize + full re-validation
  → persist                        chat/chat.repository.ts — proposals are processed BEFORE the assistant message persists; repair telemetry rides metadata.agent.repair
```
...
- **Domain LLMs run only-selected and in parallel.** Each enforces its own read-only tool allowlist and reply safety; a failed/timed-out domain degrades to a safe empty output. The **decision-maker** LLM synthesizes their outputs and emits typed proposals only — only the workout domain LLM may set a workout calorie estimate. Domain-step provider calls use a **per-turn strict structured-output wire schema** built from the typed LLM-emission schemas (`packages/types/src/llm-emission` via `ai/openai-wire-schemas.ts`); when an allowed intent lacks an emission schema the call gracefully falls back to the permissive `strict:false` shape — Zod re-parses either way.
- **Proposals are normalized, validated, and (when eligible) self-repaired BEFORE the assistant message is persisted.** ActionResolver output flows through the per-intent `ProposalNormalizationService` (trusted server-side stamping — imageRefs from turn attachments, provenance, date clamps, workout name→catalog bridge; normalizers bridge, never relax) → the full validation stack → at most one payload-only self-repair LLM call per proposal for **schema-class failures only** (never safety- or ownership-class; budget 2 repairs per turn). A repaired payload replaces only `proposedChanges` (envelope fields stay) and re-runs the same normalize + full validation stack; repair telemetry rides `metadata.agent.repair`. The chat client contract is **tolerant of invalid proposals**: the web parses the raw payload and renders an honest invalid-proposal card (human notice, Apply disabled) instead of dropping the message or showing raw validation errors.
- **SystemPlanner** is the deterministic control layer — it owns the final fan-out plan, context budget, executor modes, and tool/proposal allowlists, and caps selected domains at 3. The LLM only suggests; the catalog is the floor (router/YAML can only narrow it). It guarantees `plan.executorMode` is never deterministic by coercing any would-be deterministic mode to the fan-out default (logging `pre_ai_gate.miss`), so **every orchestrated turn fans out** — there is no orchestrator-level deterministic branch and no canned gate-miss reply.
- **Pre-AI gates** (crisis support, proposal explainer, direct chat paths) deliberately bypass the LLM and are safety/deterministic product boundaries — not duplicate routers. They are **shortcuts that fall through to the fan-out when declined**: the router's `directCommand` is telemetry-only and there is **no canned gate-miss reply**. Direct paths are now **five** (four read-only — `today_summary_read`, `nutrition_plan_read`, `weekly_progress_read`, `workout_plan_read` — plus the one narrow `mark_today_workout_done` write); **plan changes are always proposal-only**. Degradations surface a typed `turnError` (`decision_failed` after the decision-maker's one retry, or `reply_blocked`), **never fake coach prose** — the web renders an error card with Retry.
...
- Attachments are **images + document files (PDF/TXT/Markdown), context-only**: there is no recognition/classification machinery (the multimodal domain LLMs read the image content directly), **no upfront classification** (no category picker / `categorySource` declare-before-upload — the `document_file` category is `mime_inferred`), and **no upfront consent gate**. Document text is extracted lazily per-turn (12k-char cap, `ok`/`empty`/`failed` degrade) and **extracted attachment text is never persisted or logged**. **Temporary, intentional relaxation (for now):** image content — including a photo of a medical document — reaches the LLM before any consent, consciously removing the previous "medical content only when consent is granted" code floor. The `allowDocuments=false` context-budget floor (now redefined to mean **raw document-derived text** slices, not the uploaded attachment) still holds — the consent-gated `biomarkerContext` slice is exempt by design — and **no attachment path may create or parse `lab_reports` / `biomarker_readings` rows** — the chat-attachments path stays context-only and this boundary is enforced and regression-tested. **Lab-report upload/parse is now LIVE as a separate, explicit Biomarkers feature** (`apps/api/src/modules/biomarkers/*`, `apps/web/src/components/biomarkers/*`), not an attachment behavior: the user explicitly uploads a PDF/plain-text lab report under a **two-level consent model** — a structurally-required upload-time `storeAndParse` (`z.literal(true)`) plus an optional per-report `coachChat` toggle (`coachContextConsentAt`, revoke-able) — with **delete**. Extraction runs through a **dedicated OUT-OF-BAND LLM pipeline** (`packages/ai` `LabExtractionProvider` + apps/api `OpenAiLabExtractionProvider`: strict `json_schema`, closed 47-key catalog enum, temperature 0, 2 retries on 429/5xx, 60s abort, `OPENAI_MODEL_LAB_EXTRACTION` env), **separate from the chat fan-out**, with typed `failureCode`s (`file_unreadable | pdf_no_text | content_too_large | not_a_lab_report | llm_unavailable | llm_invalid_output | no_readings_extracted`) — never fake-success. Raw bytes live on the storage adapter (local filesystem in dev — production must use an access-controlled, encrypted store), **extracted document text is never persisted or logged**, readings carry a wellness-neutral "typical range"/plausibility band and pass an EN/RU unsafe-language drop, and all access is ownership-scoped. Coach context is a single bounded, consent-gated `biomarkerContext` slice (≤30 items, no reference ranges) and proposal evidence type `biomarker_reading`. This is **explicit-upload only**: there is still no LLM-recognized consent-gated **special save** (an attachment recognition signal → auto-save), which remains a deferred follow-up. The removed recognizers/classifiers, `prepare_proposal_candidates` stage, pre-upload classification/consent gate, `medical_document_save` variant, attachment proposal side-channel, and the deleted `health_documents`/`document_signals` module (with its five-scope consent model and `document_signal` evidence) must not be reintroduced (see "Removed Legacy Paths" in the pipeline doc).
...
- `packages/ai-behavior/config/domains/*.yml` — per-domain `intents[]/tools[]/signals[]/prompts[]` (workout, nutrition, health), merged by one loader. YAML can only **narrow** the capability-catalog allowlists, never widen them; YAML `prompts[]` are not directly injected into the live OpenAI fan-out domain prompts today.
