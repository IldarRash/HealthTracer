...
- Attachments are context-only: there is no recognition/classification machinery (the multimodal domain LLMs read attachment content directly). Medical attachments are consent-gated at upload by the user-declared category/document-type + MIME; a save is a consent-gated proposal — never an auto-persisted `health_document`. Do not reintroduce the recognizers/classifiers, `prepare_proposal_candidates`, or any attachment proposal side-channel (see "Removed Legacy Paths" in the pipeline doc).
