...
- **Exception — `github-agent`:** within the feature-planner workflow, the `github-agent` (`.claude/agents/github-agent.md`) is explicitly authorized to **create the feature branch, commit, push, and open the PR automatically** as the workflow's bookend, without a per-step user prompt — the user approves the overall plan instead. This supersedes the "commit/push only when the user asks" line **for that agent only**. It still must never push to `main`, never force-push, and must honor every exclusion rule below (no secrets/artifacts).
...
- **Railway applies Drizzle migrations under `packages/db/drizzle` automatically on every `health-api` deploy** via the pre-deploy command in `apps/api/railway.json`; a failed migration halts the deploy. Keep API runtime `DATABASE_URL` on private networking — the pre-deploy command uses it directly. See `docs/deployment/railway.md`.
