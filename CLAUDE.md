...
- **Pre-AI gates** (crisis support, proposal explainer, direct chat paths) deliberately bypass the LLM and are safety/deterministic product boundaries — not duplicate routers. They are **shortcuts that fall through to the fan-out when declined**: the router's `directCommand` is telemetry-only and there is **no canned gate-miss reply**. Direct paths are now **three** (two read-only — `today_summary_read`, `nutrition_plan_read` — plus the one narrow `mark_today_workout_done` write); **plan changes are always proposal-only**. Degradations surface a typed `turnError` (`decision_failed` after the decision-maker's one retry, or `reply_blocked`), **never fake coach prose** — the web renders an error card with Retry.
