...
export class ActionResolverService {
  ...
  resolveFinalDecisionOutput(
    ...
    const { finalDecision, selectedDomains, workoutCalorieEstimate } = input;
...
/** Workout-plan proposal intents that may carry a calorie estimate. */
...
/**
 * For all workout-plan proposals, ALWAYS scrub any existing calorie fields from
 * proposedChanges (to prevent decision-maker / non-workout-LLM injection), then
 * conditionally re-stamp from the trusted `estimate` when it is present.
 *
...
function scrubAndStampWorkoutCalorieEstimate(
  ...
  const result = proposals.map((proposal) => {
    ...
    if (proposal.intent === "adapt_workout_plan_from_progress") {
      return scrubAndStampFromProgressProposal(proposal, proposedChanges, estimate);
    }

    return scrubAndStampFlatWorkoutProposal(proposal, proposedChanges, estimate);
  });
