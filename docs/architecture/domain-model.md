...
Schema: `packages/db/src/schema/workouts.ts` / migration
`packages/db/drizzle/0032_wealthy_mordo.sql`. NULLs are distinct in the
`(userId, workoutPlanId, workoutPlanRevisionId, plannedDate)` unique index, so multiple
ad-hoc rows on the same day insert cleanly.
...
with live-recomputed derived values. On accept the backend recomputes the total from the
**stored** contract and a **stored, trusted** rate (sourced only from the workout domain
LLM), clamps each editable field to its stored bounds, and discards the client total
(`ProposalsService.decideProposal`); the contract and rate are stripped before any revision
is written.

Most accepted workout/nutrition proposals create new **revisions**. The **LOG
(revision-free) intents** — `log_workout_activity` (→ ad-hoc `workout_sessions` row) and
